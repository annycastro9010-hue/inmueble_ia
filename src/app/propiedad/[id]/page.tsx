...
export default function PropertyDynamicPage({ params }: { params: { id: string } }) {
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!leadName || !leadPhone) return alert("Por favor completa los datos");

    const { error } = await supabase.from('leads').insert([{
      property_id: property.id,
      client_name: leadName,
      phone: leadPhone,
      interest_level: 5 // Default High for direct contact
    }]);

    if (!error) {
      setLeadSent(true);
      setTimeout(() => {
        const WHATSAPP_URL = `https://wa.me/573001234567?text=${encodeURIComponent(`¡Hola! Soy ${leadName}. Acabo de ver "${property.title}" en la web y me interesa agendar una cita.`)}`;
  ...
  return (
    <main className="min-h-screen bg-[#062b54] text-white selection:bg-hormozi-yellow selection:text-black">
      ...
      <section className="relative h-[80vh] bg-black overflow-hidden">
        ...
        <div className="absolute top-8 left-8 z-20">
          <div className="flex items-center gap-3 px-4 py-2.5 bg-[#062b54]/80 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-hormozi-yellow">Recorrido <span className="text-white">Vivo 360°</span></span>
          </div>
        </div>
      ...
      {/* ── 3. BOTONES DE ACCIÓN (CRM TRIGGER) ── */}
      <div className="relative z-30 -mt-16 px-6 max-w-5xl mx-auto space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button 
            onClick={() => setShowLeadForm(true)}
            className="relative group overflow-hidden py-7 bg-green-500 text-white font-black rounded-3xl uppercase text-[11px] tracking-widest shadow-[0_20px_50px_rgba(34,197,94,0.3)] flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-[0.98] transition-all"
          >
          ...
          >
            <Play size={22} fill="currentColor" /> Ver Video Cinematic
          </button>
        </div>
      </div>
      ...
      <div className="px-6 md:px-12 max-w-[1400px] mx-auto py-24 space-y-40 pb-40">
        ...
        {property.video_url && (
          <section id="seccion-video" className="space-y-10">
             <div className="text-center space-y-3">
               <div className="text-hormozi-yellow text-[10px] font-black uppercase tracking-[0.4em]">Video Promocional</div>
               <h2 className="text-4xl md:text-7xl font-black uppercase italic tracking-tighter leading-none">Venta en <span className="text-hormozi-yellow">Movimiento</span></h2>
             </div>
             <div className="aspect-video max-w-6xl mx-auto rounded-[4rem] overflow-hidden border-4 border-white/5 shadow-2xl bg-black">
                <video src={property.video_url} autoPlay loop muted playsInline className="w-full h-full object-cover" />
             </div>
          </section>
        )}
        ...
        <section className="space-y-12">
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
              <h2 className="text-4xl md:text-7xl font-black uppercase italic tracking-tighter leading-none">Álbum <span className="text-hormozi-yellow">Digital</span></h2>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-16 md:gap-24 items-start">
           <div className="lg:col-span-7 space-y-12">
              <div className="space-y-6">
                 ...
                 <h1 className="text-6xl md:text-[8rem] font-extrabold uppercase italic tracking-tighter leading-[0.8]">{property.title}</h1>
                 <p className="text-xl md:text-3xl text-white/70 leading-relaxed font-medium">{property.description}</p>
              </div>
           ...
           <div className="lg:col-span-5 space-y-10 lg:sticky lg:top-32">
              <div className="bg-white p-12 md:p-16 rounded-[4rem] shadow-2xl">
                 <div className="text-black/40 font-black uppercase tracking-widest text-[10px] mb-4">Inversión Final</div>
                 <div className="text-6xl md:text-8xl font-black italic tracking-tighter text-black leading-none mb-10">${property.price?.toLocaleString()}</div>
                 <button onClick={() => setShowLeadForm(true)} className="w-full py-7 bg-[#062b54] text-white flex items-center justify-center gap-4 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] hover:bg-black transition-all">
                    Solicitar Información <ChevronRight size={20}/>
                 </button>
              </div>
        ...
        {showLeadForm && (
          ...
          >
            <motion.div 
              ...
              className="bg-[#062b54] border border-white/10 rounded-[4rem] p-10 md:p-16 max-w-xl w-full shadow-2xl relative overflow-hidden"
            >
              <button onClick={() => setShowLeadForm(false)} className="absolute top-8 right-8 text-white/20 hover:text-white transition-opacity"><X size={32}/></button>
              
              {!leadSent ? (
                <>
                  <div className="text-hormozi-yellow text-[10px] font-black uppercase tracking-[0.4em] mb-4">Solo un paso más</div>
                  <h3 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter leading-none mb-4">¿Con quién <span className="text-hormozi-yellow">hablamos?</span></h3>
                  <p className="text-white/40 text-sm mb-10 font-medium">Déjanos tus datos para enviarte el portafolio completo y agendar la cita.</p>
                  
                  <form onSubmit={handleLeadSubmit} className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase tracking-widest text-white/30 px-4">Tu Nombre</label>
                       <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-3xl py-6 pl-16 pr-6 text-lg outline-none focus:border-hormozi-yellow transition-all"
                          />
                       </div>
                    </div>
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase tracking-widest text-white/30 px-4">Tu Celular / WhatsApp</label>
                       <div className="relative">
                    ...
                    <button className="w-full py-8 bg-hormozi-yellow text-black font-black uppercase tracking-[0.2em] rounded-[2.5rem] mt-6 flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98] transition-all">
                       <Send size={20} /> Ver Portafolio y Cita
                    </button>
                  </form>
                </>
              ) : (
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-20 flex flex-col items-center">
                   <div className="w-24 h-24 bg-green-500 rounded-full flex items-center justify-center mb-10 shadow-[0_0_50px_rgba(34,197,94,0.5)]">
                      <CheckCircle size={48} className="text-white" />
                   </div>
                   <h3 className="text-4xl font-black uppercase italic tracking-tighter mb-4">¡Excelente, <span className="text-hormozi-yellow">{leadName}!</span></h3>
                   <p className="text-white/40">Te estamos redirigiendo a nuestro WhatsApp...</p>
                </motion.div>
              )}
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {lightboxOpen && (
          ...
          >
            <img src={images[currentPhoto].url} className="max-h-full max-w-full rounded-3xl border border-white/10" />
